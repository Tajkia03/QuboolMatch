...
class NIDBackOcrExtractionResult(BaseModel):
    document_detected: bool = False
    address: Optional[str] = None
    blood_group: Optional[str] = None
    image_quality: Optional[str] = None
...
class GeminiNidOcrService:
    ...
    def extract_back(self, image_data: bytes, content_type: str) -> NIDBackOcrExtractionResult:
        prompt = (
            "You are extracting text from the BACK side of a Bangladesh National ID card.\n"
            "Return only structured JSON.\n"
            "Do not guess unreadable values. If a field is unclear, use null.\n"
            "If the image is not a Bangladesh NID back side, set document_detected to false.\n"
            "Extract the printed address and blood group when readable.\n"
            "Normalize blood_group to one of A+, A-, B+, B-, AB+, AB-, O+, O- when possible.\n"
            "For warnings, return a list of short strings describing any uncertainty or quality issues.\n"
            "The image_quality value should be one of: excellent, good, fair, poor, unreadable.\n"
        )
        ...
        schema = types.Schema(
            ...
            properties={
                ...
                "blood_group": types.Schema(type=types.Type.STRING),
                "image_quality": types.Schema(type=types.Type.STRING),
                "warnings": types.Schema(
                    type=types.Type.ARRAY,
                    items=types.Schema(type=types.Type.STRING),
        ...
        try:
            ...
            try:
                return NIDBackOcrExtractionResult.model_validate(payload)
            except Exception as exc:
                raise NIDOcrExtractionError("Gemini OCR response did not match the expected schema") from exc
        except NIDOcrExtractionError:
            raise
